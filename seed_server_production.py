        ...
        for tx in block.transactions:
            ...
            for out in tx.outputs:
                if out.address == address:
                    ...
                    involved = True
            # Check inputs (sent) - need to look up prev tx
            if not tx.is_coinbase:
                for inp in tx.inputs:
                    result = blockchain.get_transaction(inp.txid)
                    if result:
                        prev_tx, _ = result
                        if inp.vout < len(prev_tx.outputs):
                            if prev_tx.outputs[inp.vout].address == address:
                                total_sent += prev_tx.outputs[inp.vout].value
                                involved = True
            if involved:
                tx_count += 1
    ...
    try:
        ...
        with lock:
            ...
            if result:
                ...
                print(f"[submit_tx] REJECTED {tx.txid[:16]}... - mempool validation failed")
                return jsonify({'error': 'Transaction rejected by mempool'}), 400
